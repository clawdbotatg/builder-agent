...
- Use \`useScaffoldReadContract\` and \`useScaffoldWriteContract\` DIRECTLY in components. Do NOT create wrapper hooks around scaffold hooks — that is an anti-pattern. The scaffold hooks ARE the abstraction.
- Use \`useScaffoldEventHistory\` for reading events.
- Hook names are: useScaffoldReadContract (NOT useScaffoldContractRead), useScaffoldWriteContract (NOT useScaffoldContractWrite).
- \`deployedContracts.ts\` is AUTO-GENERATED by \`yarn deploy\`. NEVER edit it manually.
- Add external contracts to \`externalContracts.ts\` BEFORE starting frontend work.
- Use SE2 built-in components: \`<Address/>\` for addresses, \`<AddressInput/>\` for address inputs, \`<Balance/>\` for balances.
- RainbowKit handles wallet connection and network switching — do NOT build custom NetworkGuard or wallet connect components.
...
- Deploy scripts: create \`DeployMyContract.s.sol\` in \`packages/foundry/script/\`, run with \`yarn deploy --file DeployMyContract.s.sol\`. File naming: snake_case for foundry scripts.
- Use \`yarn fork --network base\` NOT \`yarn chain\` for local dev — fork gives real protocol state.
...
- IPFS deploy requires: \`rm -rf .next out\`, \`NEXT_PUBLIC_IPFS_BUILD=true\`, polyfill-localstorage.cjs.
